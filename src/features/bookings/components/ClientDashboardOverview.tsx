...
export function ClientDashboardOverview() {
  ...
  const { userName, nextTrip, formattedPrice } = useClientDashboard(lang);
      ...
      >
        <div>
          ...
          <h1 style={{ fontSize: "var(--text-3xl)", fontWeight: 900, fontFamily: "var(--font-heading)", margin: "4px 0" }}>
            {isAr ? `مرحباً بك مجدداً، ${userName}` : `Welcome back, ${userName}`}
          </h1>
        ...
        >
          <div>
            ...
            <h3 style={{ fontSize: "17px", fontWeight: 900, margin: "10px 0 4px 0" }}>
              {isAr ? nextTrip.titleAr : nextTrip.titleEn}
            </h3>
            <p style={{ fontSize: "12px", color: "var(--color-text-secondary)", margin: 0 }}>
              {isAr ? "المرشد:" : "Guide:"} <strong>{isAr ? nextTrip.guideNameAr : nextTrip.guideNameEn}</strong> • {nextTrip.date}
            </p>
          </div>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: "16px", paddingTop: "12px", borderTop: "1px solid var(--color-border)" }}>
            <span style={{ fontSize: "14px", fontWeight: 900, color: "var(--color-saudi-green)" }}>
              {formattedPrice} ({nextTrip.travelersCount} {isAr ? "مشاركين" : "Travelers"})
            </span>
            <Link href={`/client/bookings/${nextTrip.id}`}>
